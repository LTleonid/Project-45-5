...
def handle_message(message):
    ...
    if is_yandex_music_url(url):
        ...
        if type_ == 'track':
            ...
            # Скачивание песни TODO
            download_track = best.download(f'{message.from_user.username} - {best_result_text}.mp3', 'mp3')
    ...
    if has_emoji(message.text):
        ...
        error_message = f"{current_time} | BOT -> {message.from_user.username}: Появилась ошибка - хз он на тесте. Но я должен дальше работать\n"
